/**
 * app/ru/_components/RuChrome.tsx
 * Общий каркас (навигация + CTA + футер) для русскоязычных страниц.
 * Зеркалит EnChrome; переключатели языков ведут на близнецов ID/EN.
 */

import Link from "next/link";
import Image from "next/image";

export function RuNav({ idHref, enHref }: { idHref: string; enHref: string }) {
  return (
...
export function RuCta({
  eyebrow = "Начните бесплатно",
  title = "STC AutoTrade — автоматическая торговля на вашем счёте Stockity",
  desc = "6 торговых режимов, автоматический стоп-лосс, работа 24/7 на сервере. Бесплатно для зарегистрированных участников.",
}: {
  eyebrow?: string;
  title?: string;
  desc?: string;
    ...
    <footer className="border-t border-[rgba(26,22,18,0.07)] py-6 bg-[#f6f4ef]">
      <div className="max-w-5xl mx-auto px-5 sm:px-8 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-4">
          <Link href="/en" className="text-[11px] text-[#1a1612]/65 hover:text-[#1a1612] transition-colors no-underline">English</Link>
...
export function RuSectionLabel({ children }: { children: React.ReactNode }) {
  return (
