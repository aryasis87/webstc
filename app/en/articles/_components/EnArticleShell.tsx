/**
 * app/en/articles/_components/EnArticleShell.tsx
 * English article shell — mirrors artikel/_components/ArtikelShell but with
 * English chrome and an ID-language switcher to the article's twin.
 * Content blocks (Steps, DataTable, etc.) are re-exported from the ID shell —
...
export interface EnRelatedLink {
  href: string;
  title: string;
  desc: string;
}
...
export function EnArticleShell({
  badge,
  date,
  readMins,
  breadcrumbName,
  ...
  /** path of the Indonesian twin, e.g. /artikel/robot-trading-stockity */
  idHref: string;
  title: React.ReactNode;
  lede: React.ReactNode;
  stats: { v: string; l: string }[];
  related: EnRelatedLink[];
  ctaEyebrow: string;
  ctaTitle: string;
  ctaDesc: string;
    ...
    <div className="min-h-screen bg-[#f6f4ef]">
      ...
      <div className="max-w-4xl mx-auto px-5 sm:px-8 pt-5">
        <nav aria-label="Breadcrumb">
          <ol className="flex items-center gap-1.5 text-[12px] text-[#1a1612]/65 flex-wrap">
            <li><Link href="/en" className="hover:text-[#1a1612] transition-colors no-underline">Home</Link></li>
            <li aria-hidden="true">›</li>
            <li><Link href="/en/articles" className="hover:text-[#1a1612] transition-colors no-underline">Articles</Link></li>
            <li aria-hidden="true">›</li>
      ...
      <header className="max-w-4xl mx-auto px-5 sm:px-8 pt-8 pb-10">
        <div className="flex items-center gap-3 mb-4 flex-wrap">
          <span className={`inline-flex items-center px-2.5 py-1 text-[11px] font-semibold rounded-full ${BADGE_STYLES[badge] ?? BADGE_STYLES.Education}`}>{badge}</span>
          <span className="text-[12px] text-[#1a1612]/65">{date}</span>
          <span className="text-[12px] text-[#1a1612]/65">·</span>
          <span className="text-[12px] text-[#1a1612]/65">~{readMins} min read</span>
        </div>
      ...
      <main className="max-w-4xl mx-auto px-5 sm:px-8 pb-24">
        ...
        <div className="mt-14 pt-10 border-t border-[rgba(26,22,18,0.08)]">
          <p className="text-[11px] font-semibold tracking-[0.12em] uppercase text-[#1a1612]/65 mb-5">Related Articles</p>
